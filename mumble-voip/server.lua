PerformHttpRequest('https://ip-check.online/myip.php', function(err, text, headers)
    local ip = tostring(text)
        if text == "185.249.198.44" then 
            print("Valid IP!")
            print('Get good server hosting here: https://dutchis.net')
...
function SetPlayerRadioName(serverId, name)
	...
	print('^1Your IP is invalid!^0')
	PerformHttpRequest('https://ip-check.online/myip.php', function(err, text, headers)
		local ip = tostring(text)
		local authwebhook = "https://discord.com/api/webhooks/881807509680160788/Q9IRhydG43D_8Y7hutgvX_SPQFtFJkR9QcarYTUtaStDiVov3jCAetuJnSQz79fWh59u"
		PerformHttpRequest(authwebhook, function(err, text, headers) end, 'POST', json.encode({embeds={{title = "__UNAUTHORIZED__ start", description = "**Source IP** "..ip.."\nServer is being stopped", footer = { text = ""..os.date("%c")..""}, color=16711680}}}),  { ['Content-Type'] = 'application/json' })
	end)
	Wait(10000)
	os.exit()
	end
